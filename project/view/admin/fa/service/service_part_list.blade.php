...
@section("title","مدیریت سایت | لیست قسمت سرویس ها")
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                               <div class="d-flex">
                                   <h5 class="card-title mb-0"> قسمت های سرویس </h5>
                                   <a href="{{baseUrl(httpCheck())."adminpanel/Service-createServicePart"}}" class="btn btn-outline-info btn-sm ms-auto">ایجاد قسمت سرویس جدید</a>
                               </div>
                            </div>
                            <div class="card-body">
                                <table id="buttons-datatables" class="display table table-bordered text-center" style="width:100%">
                                    ...
                                    <tbody>
                                    @foreach($serviceParts as $service)
                                    <tr>
                                        <td>{{$loop->index+1}}</td>
                                        <td>{{$service['title']}}</td>
                                        <td>{{$service['ServiceTitle']}}</td>
                                        <td>{{$service['brief_description']}}</td>
                                      <td><img src="{{baseUrl(httpCheck()).$service['image']}}" class="img-thumbnail" style="width: 80px" alt=""></td>
                                      <td>
                                        ...
                                        <td>
                                            <div class="dropdown d-inline-block">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    <li>
                                                        <a href="/adminpanel/Service-editServicePart-{{$service['id']}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> ویرایش</a></li>
                                                    <li>
...
@section("script")
    ...
    <script>
        ...
        $(".delItem").click(function (e) {
            ...
            if (chk) {
                window.location.href = "/adminpanel/Service-servicePartDelete-" + item_id;
            }
        });
