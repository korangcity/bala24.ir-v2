...
@section("title","مدیریت سایت | ویرایش پلن")
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="live-preview">
                                    ...
                                    @endif
                                    <form action="/adminpanel/Plan-editPlanProcess" method="post"
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="mb-3">
                                                    <label for="pagee" class="form-label">جایگاه مورد نظر را انتخاب کنید<span
                                                    ...
                                                    <select name="pagee[]" id="pagee" multiple="multiple"
                                                        ...
                                                        @foreach($pages as $key=>$page)
                                                            @php
                                                                ...
                                                                $selectedTrue=false;
                                                                foreach (json_decode($plan['planable_type']) as $planVal) {
                                            ...
                                            <div class="col-md-6">
                                                <div class="mb-3">
                                                    <label for="time_period" class="form-label">دوره زمانی مورد نظر را
                                                        انتخاب کنید<span class="text-danger">*</span></label>
                                                    <select name="time_period[]" multiple="multiple" id="time_period"
                                                            class="form-control js-example-disabled-multi">
                                                        <option value="">انتخاب کنید</option>
                                                        @foreach($time_periods as $ke=>$time_period)
                                                            <option value="{{$time_period['id']}}" {{in_array($time_period['id'],json_decode($plan["time_period"]))?"selected":""}}>{{$time_period['title']}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-6">
                                                <div class="mb-3">
                                                    ...
                                                    <textarea name="brief_description" class="form-control"
                                                              ...
                                                              rows="10">{{$plan['brief_description']??''}}</textarea>
                                                </div>
                                            </div>
                                        </div>
...
@section("script")
    ...
    <script>
        ...
        @php
            ...
            $desiredPositions=[];
            $pageIddds=[];
            foreach (json_decode($plan['planable_type']) as $itemPT){
                $desiredPositions[]=$positionss[$itemPT];
            }

            foreach (json_decode($plan["planable_id"]) as $itemPTId){
